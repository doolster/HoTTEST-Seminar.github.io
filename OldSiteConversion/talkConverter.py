...
for file in os.listdir('./OldSiteConversion/speakersWorkfiles'):
    with open('./OldSiteConversion/speakersWorkfiles/' + file, encoding='utf8') as f:
#with open('./temp/speakersWorkfiles/Fall2018.txt', encoding='utf8') as f:
        ...
        i = 0
        while i < len(lines):
            with open('./OldSiteConversion/NewTalkInfo/' + (lines[i+1].strip() + '-' + lines[i].strip()).replace(' ', '') + '.txt', 'w', encoding='utf8') as f:
                f.write('Term: ' + term + '\n')
